...
import { createAnimal, getAnimalByLatinName } from './db-services/animalService'
...
app.post(
    ...
    tryCatchRoute.bind(null, async (req: any, res: any): Promise<any> => {
        if (!req.session) {
            return res.status(400).json({ message: "Should provide session" });
        }
...
app.post(
    "/finalize-trade/:tradeId/:cardId",
    tryCatchRoute.bind(null, async (req: any, res: any): Promise<any> => {
        const userId = req.session.userId;
        const { tradeId, cardId } = req.params;
...
app.post('/upload', async (req: any, res: any): Promise<any> => {
    ...
    const response = await axios.request(body).catch(r => console.error(r.message))
    // const apiData = HARDCODED_API_DATA 
    const apiData = response?.data?.result
    if (!apiData?.classification?.suggestions?.length) return res.status(404).json({ message: 'not recognized' })
    const {
        name: latinName,
        details: {
            url,
    ...
    if (!foundAnimal)
        foundAnimal = await createAnimal(latinName, description)

    if (!foundAnimal) return res.status(500).json({ message: "Internal error finding and creating an animal" })

    const imageUrl = await uploadImage(image, userId).catch(console.error)
    if (!imageUrl) return res.status(500).json({ message: 'error saving the image' })
...
app.get("/check-session", async (req: any, res: Response): Promise<any> => {
    ...
    res.json({ message: "Session exists" });
});
